...
import tealbaseClient from '../client'
...
const WaitlistPopover: FC<Props> = ({}) => {
  ...
  const getGeneratedTweet = () => {
    return `Join me to experience Realtime by tealbase!%0A%0A${window.location.href}`
  }
  ...
  const onSubmit = async (values: any, { setSubmitting, resetForm }: any) => {
    ...
    setError(undefined)
    setSubmitting(true)
    const { error } = await tealbaseClient.from('waitlist').insert([{ email: values.email }])
    if (!error) {
      resetForm()
      setIsSuccess(true)
    } else {
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center justify-center space-x-2">
          <Image
            src={isDarkMode ? `/img/tealbase-dark.svg` : `/img/tealbase-light.svg`}
            alt="tealbase"
            height={20}
            width={100}
          />
      ...
      <div className="space-y-6">
        ...
        <div>
          <a
            href="https://www.producthunt.com/posts/realtime-multiplayer-by-tealbase?utm_source=badge-featured&utm_medium=badge&utm_souce=badge-realtime&#0045;multiplayer&#0045;by&#0045;tealbase"
            rel="noreferrer"
            target="_blank"
          >
            <img
              src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=339695&theme=dark"
              alt="Realtime&#0032;Multiplayer&#0032;by&#0032;tealbase - Easily&#0032;build&#0032;real&#0045;time&#0032;apps&#0032;that&#0032;enables&#0032;user&#0032;collaboration | Product Hunt"
        ...
        <div className="flex items-center gap-2">
          <Link href="https://github.com/tealbase/realtime" passHref>
