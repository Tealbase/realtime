...
const Room: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (!roomId) {
      ...
      */
      roomChannel = tealbaseClient.channel('rooms')
      ...
      */
      roomChannel = tealbaseClient.channel('rooms', { config: { presence: { key: roomId } } })
      roomChannel.on(
    ...
    return () => {
      roomChannel && tealbaseClient.removeChannel(roomChannel)
    }
  ...
  useEffect(() => {
    ...
    pingChannel = tealbaseClient.channel(`ping:${userId}`, {
    ...
    messageChannel = tealbaseClient.channel(`chat_messages:${roomId}`)
          ...
          if (e.code === 'Enter' || (e.key.length === 1 && !e.metaKey)) {
            if (!isTypingRef.current) {
              ...
              if (messageRef.current) {
                ...
                await tealbaseClient.from('messages').insert([
                  {
                    user_id: userId,
                    room_id: roomId,
    ...
    return () => {
      ...
      pingChannel && tealbaseClient.removeChannel(pingChannel)
      messageChannel && tealbaseClient.removeChannel(messageChannel)
    }
