defmodule Realtime.Extensions.CdcRlsSubscriptionsTest do
  ...
  test "create", %{conn: conn} do
    ...
    assert {:ok, [%Postgrex.Result{}]} = S.create(conn, "tealbase_realtime_test", params_list)
    ...
    assert {:error,
            "No subscription params provided. Please provide at least a `schema` or `table` to subscribe to."} =
             S.create(conn, "tealbase_realtime_test", params_list)
  ...
  test "fetch_publication_tables", %{conn: conn} do
    tables = S.fetch_publication_tables(conn, "tealbase_realtime_test")
    assert tables[{"*"}] != nil
  end
